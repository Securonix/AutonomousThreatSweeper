...
```
```text
This query detects rare downloads from the discord CDN. NOTE: This query has the potential to generate false positives and should be used in conjunction with unusual downloads from a discord cdn
```
```text
...
```
```text
Summary: This query looks for rare files written in the temp directory via VB script leveraging i.e. leveraging the Wscript.exe process.
```
```text
rg_functionality = "Endpoint Management Systems" AND sourceprocessname = WScript.exe AND filepath CONTAINS "temp" | RARE filename
```
```text
"Summary: This query looks for rare path exclusions using powershell.exe, this may be used for defence evasion. 
```
```text
...
```
```text
"Summary: This query looks for rare vbs file creations.
```
```text
...
```
```text
"Summary: This query looks for configuration preferences for Windows Defender with rare path exclusion.
```
```text
...
```
```text
"Summary: There are two parts to this query where one looks for advancedrun.exe process to stop Windows Defender and the other query looks for rare processes that are leveraged to stop Windows Defender in general. 
```
```text
...
```
```text
"Summary: This query looks for rare filenames and processes from the list of known file types indicative of the file corruptor included in the last stage. Note: This query has the potential to generate noise and should be used in conjunction with other activities related to this attack."
```
```text
...
```
```text
"Summary: This query detects activity related to removing the Defender directory leveraging powershell.exe
```
```text
...
```
```text
"Summary: This query detects the activity of removing traces of activity performed using cmd.exe and specific command line parameters.
```
```text
